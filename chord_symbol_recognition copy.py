...
def compute_pre_PRF(predicted, actual):
    ...
    TP = tf.math.count_nonzero(predicted * actual, dtype=tf.float32)
...
def comput_PRF_with_pre(TP, FP, FN):
    ...
    precision = tf.cond(pred=tf.math.is_nan(precision), true_fn=lambda: tf.constant(0.0), false_fn=lambda: precision)
    recall = tf.cond(pred=tf.math.is_nan(recall), true_fn=lambda: tf.constant(0.0), false_fn=lambda: recall)
    F1 = tf.cond(pred=tf.math.is_nan(F1), true_fn=lambda: tf.constant(0.0), false_fn=lambda: F1)
...
def train_HT():
    ...
    with tf.compat.v1.name_scope('placeholder'):
        x_p = tf.compat.v1.placeholder(tf.int32, [None, hp.n_steps, 88], name="pianoroll")
        x_len = tf.compat.v1.placeholder(tf.int32, [None], name="seq_lens")
        y_tc = tf.compat.v1.placeholder(tf.int32, [None, hp.n_steps], name="tchord")
        y_cc = tf.compat.v1.placeholder(tf.int32, [None, hp.n_steps], name="chord_change")
        dropout = tf.compat.v1.placeholder(dtype=tf.float32, name="dropout_rate")
        is_training = tf.compat.v1.placeholder(dtype=tf.bool, name="is_training")
        global_step = tf.compat.v1.placeholder(dtype=tf.int32, name='global_step')
        slope = tf.compat.v1.placeholder(dtype=tf.float32, name='annealing_slope')
    ...
    with tf.compat.v1.variable_scope("output_projection"):
        dec_input_embed = tf.compat.v1.layers.dropout(dec_input_embed, rate=dropout, training=is_training)
        chord_logits = tf.compat.v1.layers.dense(dec_input_embed, hp.n_chord_classes, name='output_dense')

    with tf.compat.v1.name_scope('loss'):
        # Chord change
        loss_cc = 1.5 * tf.compat.v1.losses.sigmoid_cross_entropy(multi_class_labels=tf.cast(y_cc, tf.float32), logits=slope*chord_change_logits, weights=source_mask)

        # Chord symbol
        loss_tc = tf.compat.v1.losses.softmax_cross_entropy(onehot_labels=tf.one_hot(y_tc, hp.n_chord_classes), logits=chord_logits, weights=target_mask)

        # Total loss
        loss = loss_cc + loss_tc
    valid = tf.reduce_sum(input_tensor=target_mask)
    ...
    update_loss = tf.compat.v1.assign(summary_loss, summary_loss + valid * [loss, loss_cc, loss_tc])
    update_valid = tf.compat.v1.assign(summary_valid, summary_valid + valid)
    mean_loss = tf.compat.v1.assign(summary_loss, summary_loss / summary_valid)
    clr_summary_loss = summary_loss.initializer
    clr_summary_valid = summary_valid.initializer
    tf.compat.v1.summary.scalar('Loss_total', summary_loss[0])
    tf.compat.v1.summary.scalar('Loss_chord_change', summary_loss[1])
    tf.compat.v1.summary.scalar('Loss_chord', summary_loss[2])
    ...
    with tf.compat.v1.name_scope('evaluation'):
        ...
        pred_cc_mask = tf.boolean_mask(tensor=pred_cc, mask=tf.cast(source_mask, tf.bool))
        y_cc_mask = tf.boolean_mask(tensor=y_cc, mask=tf.cast(source_mask, tf.bool))
        TP_cc, FP_cc, FN_cc = compute_pre_PRF(pred_cc_mask, y_cc_mask)

        # Chord
        pred_tc = tf.argmax(input=chord_logits, axis=2, output_type=tf.int32)
        pred_tc_correct = tf.equal(pred_tc, y_tc)
        pred_tc_correct_mask = tf.boolean_mask(tensor=pred_tc_correct, mask=chord_mask)
        correct = tf.reduce_sum(input_tensor=tf.cast(pred_tc_correct_mask, tf.float32))
        total = tf.cast(tf.size(input=pred_tc_correct_mask), tf.float32)
    ...
    update_count = tf.compat.v1.assign(summary_count, summary_count + [correct, total, TP_cc, FP_cc, FN_cc])
    ...
    update_score = tf.compat.v1.assign(summary_score, summary_score + [acc_tc, P_cc, R_cc, F1_cc,])
    clr_summary_count = summary_count.initializer
    clr_summary_score = summary_score.initializer
    tf.compat.v1.summary.scalar('Accuracy_tchord', summary_score[0])
    tf.compat.v1.summary.scalar('Precision_chord_change', summary_score[1])
    tf.compat.v1.summary.scalar('Recall_chord_change', summary_score[2])
    tf.compat.v1.summary.scalar('F1_chord_change', summary_score[3])
    ...
    train_writer = tf.compat.v1.summary.FileWriter(hp.graph_location + '\\train')
    test_writer = tf.compat.v1.summary.FileWriter(hp.graph_location + '\\test')
    train_writer.add_graph(tf.compat.v1.get_default_graph())
    test_writer.add_graph(tf.compat.v1.get_default_graph())
...
def train_BTC():
    ...
    with tf.compat.v1.variable_scope("output_projection"):
        input_embed = tf.compat.v1.layers.dropout(input_embed, rate=dropout, training=is_training)
        chord_logits = tf.compat.v1.layers.dense(input_embed, hp.n_chord_classes)

    with tf.compat.v1.name_scope('loss'):
        loss = tf.compat.v1.losses.softmax_cross_entropy(onehot_labels=tf.one_hot(y_tc, hp.n_chord_classes), logits=chord_logits, weights=source_mask)
    valid = tf.reduce_sum(input_tensor=source_mask)
    summary_loss = tf.Variable(0.0, trainable=False, dtype=tf.float32)
    summary_valid = tf.Variable(0, trainable=False, dtype=tf.float32)
    update_loss = tf.compat.v1.assign(summary_loss, summary_loss + valid * loss)
    update_valid = tf.compat.v1.assign(summary_valid, summary_valid + valid)
    mean_loss = tf.compat.v1.assign(summary_loss, summary_loss / summary_valid)
    clr_summary_loss = summary_loss.initializer
    clr_summary_valid = summary_valid.initializer
    tf.compat.v1.summary.scalar('Loss', summary_loss)
    ...
    update_count = tf.compat.v1.assign(summary_count, summary_count + [correct, total])
    acc_tc = summary_count[0] / summary_count[1]
    compute_score = tf.compat.v1.assign(summary_score, summary_score + acc_tc)
    clr_summary_count = summary_count.initializer
    clr_summary_score = summary_score.initializer
    tf.compat.v1.summary.scalar('Accuracy_tchord', summary_score)
