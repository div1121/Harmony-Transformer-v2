...
def get_absolute_position_encoding(length, hidden_size, min_timescale=1.0, max_timescale=1e4, start_index=0):
    ...
    signal = tf.pad(tensor=signal, paddings=[[0, 0], [0, tf.math.floormod(hidden_size, 2)]])
...
def normalize(inputs, axis=[-1], epsilon=1e-6, scope="ln", reuse=None):
    ...
    with tf.compat.v1.variable_scope(scope, reuse=reuse):
        ...
        beta = tf.compat.v1.get_variable("beta_bias", params_shape, initializer=tf.compat.v1.zeros_initializer())
        gamma = tf.compat.v1.get_variable("gamma", params_shape, initializer=tf.compat.v1.ones_initializer())
        mean = tf.reduce_mean(input_tensor=inputs, axis=axis, keepdims=True)
        variance = tf.reduce_mean(input_tensor=tf.math.squared_difference(inputs, mean), axis=axis, keepdims=True)
...
def MHA(queries, keys, values=None, n_units=None, n_heads=8, key_mask=None, forward=False, backward=False,
    ...
    with tf.compat.v1.variable_scope(scope, reuse=reuse):
        ...
        # Linear projections
        Q = tf.compat.v1.layers.dense(q, n_units, name='dense_Q')
        K = tf.compat.v1.layers.dense(k, n_units, name='dense_K')
        V = tf.compat.v1.layers.dense(v, n_units, name='dense_V')
        Q = tf.compat.v1.layers.dropout(Q, rate=dropout_rate, training=is_training)
        K = tf.compat.v1.layers.dropout(K, rate=dropout_rate, training=is_training)
        V = tf.compat.v1.layers.dropout(V, rate=dropout_rate, training=is_training)
        ...
        if not relative_position:
            ...
            K_ = tf.concat(tf.split(K, n_heads, axis=2), axis=0) # [h*N, T_k, C/h]
            V_ = tf.concat(tf.split(V, n_heads, axis=2), axis=0) # [h*N, T_k, C/h]
            R_u = tf.compat.v1.get_variable('pe_u', dtype=tf.float32, shape=[n_units], initializer=tf.compat.v1.zeros_initializer()) # [1, 1, C]
            R_v = tf.compat.v1.get_variable('pe_v', dtype=tf.float32, shape=[n_units], initializer=tf.compat.v1.zeros_initializer()) # [1, 1, C]
            ac = Q + R_u # [N, T_q, C]
            ...
            rel_pe = tf.compat.v1.layers.dense(rel_pe, n_units, name='dense_rel_pe') # [T_q, T_k, C]
            rel_pe = tf.compat.v1.layers.dropout(rel_pe, rate=dropout_rate, training=is_training)
            ...
            bd = tf.transpose(a=bd, perm=[0, 2, 1, 3]) # [h, T_q, N, C/h]
            bd = tf.matmul(bd, rel_pe, transpose_b=True) # [h, T_q, N, T_k]
            bd = tf.transpose(a=bd, perm=[0, 2, 1, 3]) # [h, N, T_q, T_k]
        ...
        if key_mask is not None:
            key_mask = tf.tile(tf.expand_dims(key_mask, 1), [n_heads, tf.shape(input=queries)[1], 1]) # [h*N, T_q, T_k]
            ...
            outputs = tf.compat.v1.where(tf.equal(key_mask, 0), paddings, outputs) # [h*N, T_q, T_k]
        ...
        if forward:
            ...
            triu = tf.tile(tf.expand_dims(triu, 0), [tf.shape(input=outputs)[0], 1, 1]) # [h*N, T_q, T_k]
            paddings = tf.ones_like(triu) * (-2**32+1)
            outputs = tf.compat.v1.where(tf.equal(triu, 0), paddings, outputs) # [h*N, T_q, T_k]

        elif backward:
            diag_vals = tf.ones_like(outputs[0, :, :]) # [T_q, T_k]
            tril = tf.linalg.band_part(diag_vals, -1, 0) # lower triangular mask, [T_q, T_k]
            tril = tf.tile(tf.expand_dims(tril, 0), [tf.shape(input=outputs)[0], 1, 1]) # [h*N, T_q, T_k]
            paddings = tf.ones_like(tril) * (-2**32+1)
            outputs = tf.compat.v1.where(tf.equal(tril, 0), paddings, outputs) # [h*N, T_q, T_k]
        ...
        # Output projection
        outputs = tf.compat.v1.layers.dense(outputs, n_units, name='dense_O') # Concat(head 1 , ..., head h )W_O
        outputs = tf.compat.v1.layers.dropout(outputs, rate=dropout_rate, training=is_training) # [N, T_q, C]
...
def FFN(inputs, n_units=[512, 128], kernel_size=1, activation_function=tf.nn.relu, dropout_rate=0.0, is_training=False, scope="FFN", reuse=None):
    ...
    '''
    with tf.compat.v1.variable_scope(scope, reuse=reuse):
        outputs = tf.compat.v1.layers.conv1d(inputs=inputs, filters=n_units[0], kernel_size=kernel_size, activation=activation_function, use_bias=True, padding='same')
        outputs = tf.compat.v1.layers.dropout(outputs, rate=dropout_rate, training=is_training)
        outputs = tf.compat.v1.layers.conv1d(inputs=outputs, filters=n_units[1], kernel_size=kernel_size, activation=None, use_bias=True, padding='same')
        outputs = tf.compat.v1.layers.dropout(outputs, rate=dropout_rate, training=is_training)
        # Residual connection
        outputs += inputs
        # Normalization
...
def convFFN(inputs, n_units=[128, 128], activation_function=tf.nn.relu, dropout_rate=0, is_training=True, scope="convFFN", reuse=None):
    with tf.compat.v1.variable_scope(scope, reuse=reuse):
        outputs = tf.compat.v1.layers.conv1d(inputs=inputs, filters=n_units[0], kernel_size=3, activation=activation_function, use_bias=True, padding='same')
        outputs = tf.compat.v1.layers.dropout(outputs, rate=dropout_rate, training=is_training)
        outputs = tf.compat.v1.layers.conv1d(inputs=outputs, filters=n_units[1], kernel_size=3, activation=activation_function, use_bias=True, padding='same')
        outputs = tf.compat.v1.layers.dropout(outputs, rate=dropout_rate, training=is_training)
        # Residual connection
        outputs += inputs
        # Normalization
...
def chord_block_compression(hidden_states, chord_changes, compression='mean'):
    ...
    num_blocks = tf.reduce_max(input_tensor=block_ids, axis=1) + 1 # batched number of blocks
    max_steps = tf.reduce_max(input_tensor=num_blocks) # max number of blocks

    if compression == 'mean':
        segment_compress_and_pad = lambda x: tf.pad(tensor=tf.math.segment_mean(data=x[0], segment_ids=x[1]), paddings=[[0, max_steps - x[2]], [0, 0]], constant_values=0.0)
    else:  # 'compression == sum'
        segment_compress_and_pad = lambda x: tf.pad(tensor=tf.math.segment_sum(data=x[0], segment_ids=x[1]), paddings=[[0, max_steps - x[2]], [0, 0]], constant_values=0.0)
...
def BTC(x, source_mask, dropout_rate, is_training, hyperparameters):
    ...
    input = x
    with tf.compat.v1.variable_scope("encoder_input_embedding"):
        input_embed = tf.compat.v1.layers.dense(input, hyperparameters.input_embed_size) # [n_batches, n_steps, input_embedding_size]

    with tf.compat.v1.name_scope("encoder_positional_encoding"):
        input_embed += get_absolute_position_encoding(tf.shape(input=input_embed)[1], tf.shape(input=input_embed)[2])
        input_embed = tf.compat.v1.layers.dropout(input_embed, rate=dropout_rate, training=is_training)

    with tf.compat.v1.variable_scope("forward_encoding"):
    ...
    with tf.compat.v1.variable_scope("backward_encoding"):
    ...
    input_embed = tf.compat.v1.layers.dense(input_embed, 2*hyperparameters.input_embed_size)
...
def HT(x, source_mask, target_mask, slope, dropout_rate, is_training, hyperparameters):
    ...
    with tf.compat.v1.variable_scope("encoder_input_embedding"):
        enc_input_embed = tf.compat.v1.layers.dense(input, hyperparameters.input_embed_size) # [n_batches, n_steps, input_embedding_size]

    with tf.compat.v1.name_scope("encoder_positional_encoding"):
        enc_input_embed += get_absolute_position_encoding(tf.shape(input=enc_input_embed)[1], tf.shape(input=enc_input_embed)[2])
        enc_input_embed = tf.compat.v1.layers.dropout(enc_input_embed, rate=dropout_rate, training=is_training)

    with tf.compat.v1.variable_scope("encoder"):
        enc_weights = tf.nn.softmax(tf.compat.v1.get_variable('enc_weights_of_layers', dtype=tf.float32, shape=[hyperparameters.n_layers + 1], initializer=tf.compat.v1.initializers.zeros())) # [n_layers]
    ...
    with tf.compat.v1.name_scope("chord_change_prediction"):
        chord_change_logits = tf.squeeze(tf.compat.v1.layers.dense(enc_input_embed, 1), axis=-1) # [n_batches, n_steps]
    ...
    with tf.compat.v1.variable_scope("decoder_input_embedding"):
        dec_input_embed = tf.compat.v1.layers.dense(input, hyperparameters.input_embed_size) # [n_batches, n_steps, input_embed_size]

    with tf.compat.v1.variable_scope("input_embedding_regionalization"):
    ...
    with tf.compat.v1.name_scope("decoder_positional_encoding"):
        dec_input_embed += get_absolute_position_encoding(tf.shape(input=dec_input_embed)[1], tf.shape(input=dec_input_embed)[2])
        dec_input_embed = tf.compat.v1.layers.dropout(dec_input_embed, rate=dropout_rate, training=is_training)

    with tf.compat.v1.variable_scope("decoder"):
        dec_weights = tf.nn.softmax(tf.compat.v1.get_variable('dec_weights_of_layers', dtype=tf.float32, shape=[hyperparameters.n_layers + 1], initializer=tf.compat.v1.initializers.zeros())) # [n_layers]
...
def HTv2(x, source_mask, target_mask, slope, dropout_rate, is_training, hyperparameters):
    ...
    with tf.compat.v1.name_scope("chord_change_prediction"):
        chord_change_logits = tf.squeeze(tf.compat.v1.layers.dense(enc_input_embed, 1), axis=-1) # shape = [n_batches, n_steps]
    ...
    with tf.compat.v1.name_scope("decoder_positional_encoding"):
        ...
        dec_pe_batch = tf.tile(dec_pe, [tf.shape(input=dec_input_embed)[0],1,1])
